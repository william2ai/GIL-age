...
def split_data(labels, val_prop, test_prop, seed):
    ...
    # 获取数据点的总数
    nb_points = labels.size

    # 生成所有索引
    all_idx = np.arange(nb_points)

    # 打乱索引
    np.random.shuffle(all_idx)

    # 计算验证集和测试集的大小
    nb_val = int(np.round(val_prop * nb_points))
    nb_test = int(np.round(test_prop * nb_points))
...
def load_data_nc(dataset, use_feats, data_path, split_seed):
    if dataset in ['cora', 'pubmed', 'citeseer']:
        ...
        if dataset == 'disease_nc':
            ...
            adj, features, labels = load_eeg_data(dataset, use_feats, data_path)
            val_prop, test_prop = 0.1, 0.15   #待定
        else:
            raise FileNotFoundError('Dataset {} is not supported.'.format(dataset))
...
def load_eeg_data(dataset_str, use_feats, data_path):
