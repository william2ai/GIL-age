...
class Decoder(nn.Module):
    ...
    #     return probs
    def decode(self, x, adj):
        age = None  # Provide a default value
        if self.decoder_name is not None:
            input = (x, adj)
            age = self.forward(input)
        else:
            if self.decode_adj:
                input = (x, adj)
                age = self.cls.forward(input)
            else:
                age = self.cls.forward(x)
        return age
...
class MLP(nn.Module):
    def __init__(self):
        ...
        # First Hidden Layer: 18 (input size) -> 37
        self.fc1 = nn.Linear(18, 37)
        # Second Hidden Layer: 37 -> 75
        self.fc2 = nn.Linear(37, 18)
        # Output Layer: 75 -> 1
        self.fc3 = nn.Linear(18, 1)

    def forward(self, x):
        x = x.to(torch.float32)
        # Applying ReLU activation function after each hidden layer
        x = torch.relu(self.fc1(x))
        x = torch.relu(self.fc2(x))
        # No activation function after output layer
        x = torch.sigmoid(self.fc3(x)) * 2  # 应用sigmoid并调整范围到0-2
        return x
...
class DualDecoder(Decoder):
    def __init__(self, c, args):
        ...
        if args.dataset == 'pubmed':
            ...
            #                      lambda x: x)
            self.cls_e = GATConv(self.in_features, 1, 8, False, args.alpha, args.dropout, args.bias,
                                torch.relu)
            self.cls_h = HGATConv(self.manifold, self.in_features, 1, heads=8, concat=False,
                                  negative_slope=args.alpha, dropout=args.dropout,
                                  bias=args.bias, act=act, atten=args.atten, dist=args.dist)
    ...
    def forward(self, input):
        ...
        adj = input[1]
        '''Euclidean probs'''
        age_e_emb, _ = self.cls_e((x_e, adj))

        '''Hyper probs'''
        age_h_emb = self.cls_h((x, adj))

        age_e_emb = age_e_emb.T
        age_h_emb = age_h_emb.T

        age_e = self.net1(age_e_emb)
        age_h = self.net2(age_h_emb)
        # x = x.unsqueeze(-2)
        ...
        # self.weight_e = 0.2
        self.weight_e = nn.Parameter(torch.tensor(0.5))
        weight_h = 1 - self.weight_e

        age = age_h * weight_h + age_e * self.weight_e
        return super(DualDecoder, self).forward(age)
