...
class NCModel(BaseModel):
    ...
    def compute_metrics(self, embeddings, data, single_adj, single_features, idx, args):

        output = self.decode(embeddings, single_adj, idx)
        # print(output)
        # print(data['labels'][idx])

        loss = F.l1_loss(output.squeeze(0), data['labels'][idx])

        output = output.squeeze(0)
        mae = F.l1_loss(10**output, 10**data['labels'][idx])
        # print(10 ** output)
        # print(10 ** data['labels'][idx])
        # mae = F.l1_loss(torch.exp(output), torch.exp(data['labels'][idx]))
