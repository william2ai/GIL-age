...
class HGATConv(MessagePassing):
    ...
    def forward(self, input):
        ...
        out = self.propagate(edge_index, x=log_x, num_nodes=x.size(0), original_x=x, adj = adj)
    ...
    def message(self, edge_index_i, x_i, x_j, num_nodes, original_x_i, original_x_j, edge_index, adj):
        ...
        if self.atten:
            ...
            if self.dist:  # Compute distance
                ...
                alpha = alpha * dist
            alpha = F.leaky_relu(alpha*a_uv, self.negative_slope)
