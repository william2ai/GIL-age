...
class GATConv(MessagePassing):
    ...
    def forward(self, input):
        ...
        out = self.propagate(edge_index, x=x, num_nodes=x.size(0), adj=adj)
        out = self.act(out)
        return out, adj

    def message(self, edge_index_i, x_i, x_j, num_nodes, edge_index, adj):
        # Compute attention coefficients.
        # x_i = pmath.parallel_transport0(x_i, x_i)
        # x_j = pmath.parallel_transport0(x_i, x_j)

        edge_index_u = edge_index[1]
        edge_index_v = edge_index[0]
        a_uv = adj[edge_index_u, edge_index_v]
        a_uv = a_uv.unsqueeze(1)

        alpha = (torch.cat([x_i, x_j], dim=-1) * self.att).sum(dim=-1)
        alpha = F.leaky_relu(alpha * a_uv, self.negative_slope)  # 包含 A_uv 或 A_wv
        # alpha = F.leaky_relu(alpha , self.negative_slope)  # 包含 A_uv 或 A_wv
